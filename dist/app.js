...
function newSet(ex,previous){return {id:uid(),weight:ex.loadType==='bodyweight'?'0':previous?.weight||'',reps:previous?.reps||'',done:false,failure:false};}
...
function completedCount(entries){return entries.reduce((sum,entry)=>sum+entry.sets.filter(set=>set.done).length,0);}
...
function renderExercise(entry){
  ...
  const heads=el('div','set-grid set-labels');heads.setAttribute('aria-hidden','true');['№','Вес, кг','Повторы','Готово'].forEach(text=>heads.append(el('span','',text)));container.append(heads);
  ...
  entry.sets.forEach((set,index)=>{
    ...
    const check=action(set.done?'✓':'○','check',()=>{
      if(!set.done){const weight=parseNumber(set.weight),reps=parseNumber(set.reps);if(!Number.isFinite(weight)||weight<0||weight>100000||!Number.isInteger(reps)||reps<1||reps>10000){error.textContent='Введи вес от 0 кг и целое число повторов от 1.';weightInput.setAttribute('aria-invalid',String(!Number.isFinite(weight)||weight<0||weight>100000));repsInput.setAttribute('aria-invalid',String(!Number.isInteger(reps)||reps<1||reps>10000));return;}set.weight=String(weight);set.reps=String(reps);}
      set.done=!set.done;check.textContent=set.done?'✓':'○';check.setAttribute('aria-pressed',String(set.done));error.textContent='';persist();updateWorkoutCounts();
    });check.setAttribute('aria-label',`${ex.name}, подход ${index+1}: выполнен`);check.setAttribute('aria-pressed',String(set.done));
    ...
    for(const [node,key] of [[weightInput,'weight'],[repsInput,'reps']])node.addEventListener('input',()=>{set[key]=node.value;set.done=false;check.textContent='○';check.setAttribute('aria-pressed','false');node.removeAttribute('aria-invalid');error.textContent='';persist();updateWorkoutCounts();});
    line.append(weightInput,repsInput,check);container.append(line);
...
function renderDashboard(){
  ...
  for(const [id,label] of [['month','Месяц'],['day','День']]){const button=action(label,'',()=>{view.period=id;if(id==='month')view.month=view.day.slice(0,7);else if(!view.day.startsWith(view.month))view.day=`${view.month}-01`;render();});button.setAttribute('aria-pressed',String(view.period===id));periods.append(button);}main.append(periods);
  const dayMode=view.period==='day';const date=input(dayMode?view.day:view.month,dayMode?'date':'month');date.setAttribute('aria-label','Период дашборда');date.addEventListener('change',()=>{if(dayMode&&validDate(date.value))view.day=date.value;else if(!dayMode&&/^\d{4}-\d{2}$/.test(date.value))view.month=date.value;render();});const dateRow=row(el('span','small',dayMode?'Выбрать день':'Выбрать месяц'),date);dateRow.classList.add('date-row');main.append(dateRow);
...
function drawChart(svg,detail){
  ...
  if(!points.length){add('text',{x:width/2,y:height/2,'text-anchor':'middle'},'Пока нет результатов за месяц');detail.textContent='Выбери другое упражнение или месяц.';return;}
...
function openTechnique(ex,onBack){
  ...
  body.append(action('Мои заметки и настройки','secondary full spaced',()=>exerciseSettings(ex,onBack)));if(onBack)body.append(action('Назад к каталогу','quiet full',onBack));showSheet(exerciseName(ex),body);
}
...
function exerciseForm(base,options={}){
  ...
  const submit=el('button','primary full',base?'Добавить тренажёр':'Добавить упражнение');submit.type='submit';form.append(submit,action('Назад','quiet',()=>openCatalog(options)));
  form.addEventListener('submit',async event=>{event.preventDefault();if(!name.value.trim())return;if(!validUrl(url.value)){notify('Ссылка должна начинаться с https://');return;}const id=uid(),ex={id,name:name.value.trim(),group:group.value,equipment:equipment.value.trim(),loadType:load.value,baseId:base?.baseId||id,machine:machine.value.trim(),setup:'',notes:notes.value.trim(),sourceUrl:url.value.trim(),favorite:true};const next=structuredClone(data);next.exercises.push(ex);if(await commit(next)){if(options.onPick)options.onPick(id);else{openCatalog(options);notify('Добавлено в каталог.');}}});showSheet(base?'Другой тренажёр':'Своё упражнение',form);
}
